...
p_name=''
while True:
    keys = kb.getKeys() #(['a', 'z', 'return']) need to type out letters
    for key in keys:
...
On each trial the predominant movement of the dots will either be towards the left, right, up, or down.
...
DOT_SPEED = 0.1  # degrees/frame
DOT_FIELD_SIZE = 2  # degrees
COHERENCE_LEVELS = [0.05, 0.1, 0.2, 0.4, 0.8]  # Fixed coherence levels; can change to 1 to make it easier to see 
DIRECTIONS = [0, 90, 180, 270]  # Right (0°) and Left (180°)
    ...
    for direction in DIRECTIONS:
        ...
        for rep in range(0,trialn//10):  
            trial_list.append({
                'coherence': coherence,
                'direction': direction,
                'correct_response': 'left' if direction == 180 else 'right' if direction == 0 else 'up' if direction == 90 else 'down'
...
for trial in trials:
    ...
    while response is None:
        ...
        # Check for response
        keys = kb.getKeys(['left', 'right', 'up', 'down', 'escape'], waitRelease=False)
        if keys:
